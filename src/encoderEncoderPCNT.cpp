...
/*
void encoderEncoderPCNT::addEncoders(int &val)
{
	//_pulsePinA = pinA;
	//_pulsePinB = pinB;
	...
	// Create unit
	if (pcnt_new_unit(&ucfg, &_pcntA) != ESP_OK){ val = 5; return; }
	if (pcnt_new_unit(&ucfg, &_pcntB) != ESP_OK){ val = 6; return; }
	
	// Glitch filter setup
	pcnt_glitch_filter_config_t filter_cfg = {
		.max_glitch_ns = glitch_time,
	};
	if (pcnt_unit_set_glitch_filter(_pcntA, &filter_cfg) != ESP_OK){ val = 7; return; }
	if (pcnt_unit_set_glitch_filter(_pcntB, &filter_cfg) != ESP_OK){ val = 8; return; }
	...
	if (pcnt_new_channel(_pcntA, &chan_a_cfg, &_chanA) != ESP_OK){ val = 9; return; }
	...
	if (pcnt_new_channel(_pcntB, &chan_b_cfg, &_chanB) != ESP_OK){ val = 10;  return; }
